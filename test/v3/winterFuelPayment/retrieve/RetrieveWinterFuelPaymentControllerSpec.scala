...
class RetrieveWinterFuelPaymentControllerSpec extends ControllerBaseSpec with ControllerTestRunner {
  ...
  private val requestData: RetrieveWinterFuelPaymentRequestData =
    RetrieveWinterFuelPaymentRequestData(parsedNino, TaxYear.fromMtd(taxYear), `hmrc-held`)

  "RetrieveWinterFuelPaymentController" should {
    "return a successful response with status 200 (OK)" when {
      "given a valid request" in new Test {
        ...
        runOkTest(
          expectedStatus = OK,
          maybeExpectedResponseBody = Some(responseMtdJson)
        )
      }
    }
  ...
  trait Test
    ...
    val controller: RetrieveWinterFuelPaymentController = new RetrieveWinterFuelPaymentController(
      ...
      validatorFactory = mockRetrieveWinterFuelPaymentValidatorFactory,
      service = mockRetrieveWinterFuelPaymentService,
      cc = cc,
