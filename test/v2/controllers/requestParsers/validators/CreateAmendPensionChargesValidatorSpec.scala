...
class CreateAmendPensionChargesValidatorSpec extends UnitSpec with MockAppConfig {

  private val validNino    = "AA123456A"
  private val validTaxYear = "2021-22"

  class Test extends MockAppConfig {
    val validator = new CreateAmendPensionChargesValidator(mockAppConfig)
  ...
  "Running a validation" should {
    "return no errors" when {
      "a valid request is supplied" in new Test {
        validator.validate(CreateAmendPensionChargesRawData(validNino, validTaxYear, AnyContentAsJson(fullValidJson))) shouldBe Nil
      }
    }

    "return no errors when pensionContributions updated" when {
      "a valid request is supplied" in new Test {
        validator.validate(CreateAmendPensionChargesRawData(validNino, validTaxYear, AnyContentAsJson(fullValidJsonUpdated))) shouldBe Nil
      }
    }

    "return country errors" when {
      "multiple country codes are invalid for multiple reasons" in new Test {
        validator.validate(CreateAmendPensionChargesRawData(validNino, validTaxYear, AnyContentAsJson(fullJsonWithInvalidCountries))) shouldBe List(
      ...
      "multiple country codes are invalid format" in new Test {
        validator.validate(
          CreateAmendPensionChargesRawData(validNino, validTaxYear, AnyContentAsJson(fullJsonWithInvalidCountryFormat("INVALID")))) shouldBe List(
          CountryCodeFormatError.copy(paths = Some(
            Seq(
      ...
      "multiple country codes are invalid rule" in new Test {
        validator.validate(
          CreateAmendPensionChargesRawData(validNino, validTaxYear, AnyContentAsJson(fullJsonWithInvalidCountryFormat("BOB")))) shouldBe List(
          RuleCountryCodeError.copy(paths = Some(
            Seq(
              "/pensionSchemeOverseasTransfers/overseasSchemeProvider/2/providerCountryCode",
              "/overseasPensionContributions/overseasSchemeProvider/2/providerCountryCode"
    ...
    "return big decimal error" when {
      "an too large number is supplied" in new Test {
        validator.validate(CreateAmendPensionChargesRawData(validNino, validTaxYear, AnyContentAsJson(fullJson(999999999999.99)))) shouldBe List(
          MtdError(
            "FORMAT_VALUE",
      ...
      "an too small number is supplied" in new Test {
        validator.validate(CreateAmendPensionChargesRawData(validNino, validTaxYear, AnyContentAsJson(fullJson(-69420.00)))) shouldBe List(
          MtdError(
            "FORMAT_VALUE",
            "The field should be between 0 and 99999999999.99",
            BAD_REQUEST,
    ...
    "return a path parameter error" when {
      "an invalid nino is supplied" in new Test {
        validator.validate(CreateAmendPensionChargesRawData("badNino", validTaxYear, AnyContentAsJson(fullJson))) shouldBe List(NinoFormatError)
      }
      "an invalid tax year is supplied" in new Test {
        validator.validate(CreateAmendPensionChargesRawData(validNino, "2000", AnyContentAsJson(fullJson))) shouldBe List(TaxYearFormatError)
      }
      "the taxYear range is invalid" in new Test {
        validator.validate(CreateAmendPensionChargesRawData(validNino, "2021-24", AnyContentAsJson(fullJson))) shouldBe List(
          RuleTaxYearRangeInvalidError)
      }
      "all path parameters are invalid" in new Test {
        validator.validate(CreateAmendPensionChargesRawData("badNino", "2000", AnyContentAsJson(fullJson))) shouldBe List(
          NinoFormatError,
          TaxYearFormatError)
      }
      "return a RULE_INCORRECT_OR_EMPTY_BODY_SUBMITTED error" when {
        "an empty body" in new Test {
          validator.validate(CreateAmendPensionChargesRawData(validNino, validTaxYear, body = AnyContentAsJson(emptyJson))) shouldBe List(
