  ...
  "Rule annual allowance" when {
    "only is annual allowance reduced provided" must {
      "return a validation error when only isOnlyAnnualAllowanceReduced" in {
        RuleIsAnnualAllowanceReducedValidation.validate(isAnnualAllowanceReduced = true, Some(false), Some(false)) shouldBe List(
    ...
    "two are provided there should be no errors" must {
      "return no errors with isAnnualAllowanceReduced and moneyPurchasedAllowance" in {
        RuleIsAnnualAllowanceReducedValidation.validate(isAnnualAllowanceReduced = true, Some(false), Some(true)) shouldBe Nil
      }
      "return no errors with is annualAllowanceReduced and moneyPurchasedAllowance" in {
        RuleIsAnnualAllowanceReducedValidation.validate(isAnnualAllowanceReduced = true, Some(true), Some(false)) shouldBe Nil
      }
