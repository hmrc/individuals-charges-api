...
import api.controllers.requestParsers.validators.validations.{CountryCodeValidation, NoValidationErrors}
  ...
  "CountryCodeValidation" when {
    "validate" must {
      "return an empty list for a valid country code" in {
        CountryCodeValidation.validate("CYM") shouldBe NoValidationErrors
      }
    ...
    "validateOptional" must {
      ...
      "validate correctly for some invalid rule country code" in {
        CountryCodeValidation.validateOptional(Some("SBT")) shouldBe List(RuleCountryCodeError)
      }
    }
  }
